...
AForm::AForm() : kname_("default"), is_signed_(false), kgradeToSigned_(0),
				kgradeToExecute_(0)
{
	std::cout << BLUE << "AForm default constructor called." << RESET
		<< std::endl;
}
...
AForm::~AForm()
{
	std::cout << RED << "[AForm] destructor called for " << RESET << kname_
		<< std::endl;
}
...
{
	return "Grade is too high in AForm";
	// return exception::what();
}

const char* AForm::GradeTooLowException::what() const throw()
{
	return "Grade is too low in AForm";
	// return exception::what();
}
